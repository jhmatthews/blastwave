...
def get_shock_radii_and_properties(D, data, i):
    ...
    if np.sum(D.select_ism) > 0:
        data["r_forward"][i] = np.max(D.x1[D.select_ism]) * length_conversion
    ...
    data["r_reverse"][i] = np.max(D.x1[D.select_blob]) * length_conversion
...
print ("Plotting and saving profiles for katie sim analogue")
#plot_and_save_profiles("test_katie_analogue/", nrange, load=False, gamma0=2.5, plot=False)  
#plot_and_save_profiles("katie_analogue/", nrange, load=False, gamma0=2.5, plot=False)      
